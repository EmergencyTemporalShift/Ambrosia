...
pub fn animate_platformer_character(
    ...
    //TnuaAnimatingState is a helper for controlling animations. The user system must provide it
    // with an enum every frame. This enum describes the character's current state. The helper then
    // checks if the enum variant has changed. A change usually means the system should start a new
    // animation. If the variant remained the same, the system should not change the animation.
    // However, it may still adjust the animation speed based on the enum's payload.
    ...
    for (mut animating_state, controller, handler) in animations_handlers_query.iter_mut() {
        ...
        // `update_by_discriminant` which will decide whether we need to switch the
        // animation.
