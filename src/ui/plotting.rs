...
impl PlotSource {
    ...
    pub fn show(&self, entity: Entity, ui: &mut Ui) {
...
pub fn make_update_plot_data_system<V: Component>(
    ...
    (move |mut query_physics: Query<(&mut PlotSource, &Transform, &V)>| {
        for (mut plot_source, transform, velocity) in query_physics.iter_mut() {
